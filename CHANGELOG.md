...
- Viewer: A new button now can force full-frame rendering instead of the visible portion. This may be useful to remove borders artifacts when panning/zooming during playback
- Fix a bug where the Glow node would not work correctly
- Windows: Fix a bug where the 32-bit version would crash on launch with AMD Graphics Cards
...
- Tracker: when clicking "Set Input RoD" in the From points of the corner pin, automatically re-compute the To points overall all keyframes
