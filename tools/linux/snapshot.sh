...
if [ "$CURRENT_BRANCH" != "$MASTER_BRANCH" ]; then
  ...
  if [ "$BUILD_NATRON" = "1" -o "$BUILD_IO" = "1" -o "$BUILD_MISC" = "1" -o "$BUILD_ARENA" = "1" -o "$BUILD_CV" = "1" ]; then
    ...
    UNIT_TEST=$DOTEST NO_INSTALLER=$NOMKINSTALL GIT_BRANCH=$CURRENT_BRANCH NATRON_LICENSE=$NATRON_LICENSE OFFLINE_INSTALLER=1 SYNC=$DO_SYNC BUILD_CONFIG=SNAPSHOT sh build.sh
  fi
fi
