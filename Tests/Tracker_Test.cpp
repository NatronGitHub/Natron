...
static void padd(std::vector<Point>& v, double x, double y)
{
    v.resize(v.size() + 1);
    Point& p = v.back();
    p.x = x;
...
static void makeMatFromVectorOfPoints(const std::vector<Point>& x, openMVG::Mat& M)
{
    M.resize(2, x.size());
...
template <typename MODELTYPE>
int Score(const typename MODELTYPE::Model & model,
...
static openMVG::Vec4 makeSimilarity(double rotationDegrees, double scale, double tx, double ty)
{
    openMVG::Vec4 r;
...
static void similarityApply(const openMVG::Vec4& S, const openMVG::Vec2& X1, openMVG::Vec2* X2)
{
    openMVG::Mat2 m;
    m(0,0) = S(3); m(0,1) = -S(2);
    m(1,0) = S(2); m(1,1) = S(3);
...
static Point similarityApply(const openMVG::Vec4& S, const Point& x1)
{
    openMVG::Vec2 p1;
    p1(0) = x1.x;
    p1(1) = x1.y;
...
static void computeSimilarity(int w1, int h1, int w2, int h2, const openMVG::Vec4& S, const std::vector<Point>& x1, std::vector<Point>* x2, openMVG::Vec4* model, InliersVec* inliers)
{
    x2->resize(x1.size());
    for (std::size_t i = 0; i < x1.size(); ++i) {
...
static void testSimilarity(const std::vector<Point>& x1)
{
