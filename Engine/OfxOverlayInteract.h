...
class OfxOverlayInteract :  public OFX::Host::ImageEffect::OverlayInteract, public NatronOverlayInteractSupport
{
...
public:
    ...
    /*Swaps the buffer of the attached viewer*/
    virtual OfxStatus swapBuffers() { return n_swapBuffers(); }
    
    /*Calls updateGL() on the attached viewer*/
    virtual OfxStatus redraw() { return n_redraw(); }
    
    /// hooks to kOfxInteractPropViewportSize in the property set
    /// this is actually redundant and is to be deprecated
    virtual void getViewportSize(double &width, double &height) const { n_getViewportSize(width, height); }
    
    // hooks to live kOfxInteractPropPixelScale in the property set
    virtual void getPixelScale(double& xScale, double& yScale) const { n_getPixelScale(xScale, yScale); }
    
    // hooks to kOfxInteractPropBackgroundColour in the property set
    virtual void getBackgroundColour(double &r, double &g, double &b) const { n_getBackgroundColour(r, g, b); }
    
#ifdef OFX_EXTENSIONS_NUKE
    // hooks to kOfxPropOverlayColour in the property set
    virtual void getOverlayColour(double &r, double &g, double &b) const  { n_getOverlayColour(r, g, b); }
#endif
};
    
class OfxParamOverlayInteract : public OFX::Host::Interact::Instance, public NatronOverlayInteractSupport
{
    
public:
    
    OfxParamOverlayInteract(KnobI* knob,OFX::Host::Interact::Descriptor &desc, void *effectInstance);
