...
MemoryFile::MemoryFile(const std::string& filepath, FileOpenMode open_mode)
...
MemoryFile::MemoryFile(const std::string& filepath, size_t size,FileOpenMode open_mode)
: _imp(new MemoryFilePrivate(filepath))
{
    _imp->openInternal(open_mode);
    resize(size);
...
void MemoryFile::open(const std::string& filepath,FileOpenMode open_mode)
{
...
void MemoryFilePrivate::openInternal(MemoryFile::FileOpenMode open_mode)
{
#if defined(__NATRON_UNIX__)
    /*********************************************************
     ********************************************************
...
char* MemoryFile::data() const { return _imp->data; }

size_t MemoryFile::size() const { return _imp->size; }

std::string MemoryFile::path() const {return _imp->path;}
...
void MemoryFilePrivate::closeMapping()
{
#if defined(__NATRON_UNIX__)
    ::munmap(data,size);
    ::close(file_handle);
