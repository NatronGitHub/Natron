                                                ...
                                                const std::map<std::string, std::string>& oldNewScriptNamesMapping)
{
    ...
    for (std::vector<KnobIPtr>::const_iterator it = children.begin(); it != children.end(); ++it) {
        (*it)->restoreLinks(allNodes, oldNewScriptNamesMapping);
        KnobGroupPtr isGroup = boost::dynamic_pointer_cast<KnobGroup>(*it);
        if (isGroup) {
            restoreKnobLinksRecursive(allNodes, isGroup, oldNewScriptNamesMapping);
        }
    }
}
                        ...
                        const std::map<std::string, std::string>& oldNewScriptNamesMapping)
{
    ...
    for (std::vector<KnobIPtr>::iterator it = knobs.begin(); it != knobs.end(); ++it) {
        ...
        if (group) {
            _imp->restoreKnobLinksRecursive(allNodes, group, oldNewScriptNamesMapping);
        }
    }
}
