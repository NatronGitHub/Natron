...
void
TrackerFrameAccessor::getEnabledChannels(bool* r, bool* g, bool* b) const
{
...
double
TrackerFrameAccessor::invertYCoordinate(double yIn, double formatHeight)
{
    return formatHeight - 1 - yIn;
}

void
TrackerFrameAccessor::convertLibMVRegionToRectI(const mv::Region& region, int formatHeight, RectI* roi)
{
    roi->x1 = region.min(0);
    roi->x2 = region.max(0);
...
// Not used in LibMV
bool
TrackerFrameAccessor::GetClipDimensions(int /*clip*/, int* /*width*/, int* /*height*/)
{
    return false;
}
