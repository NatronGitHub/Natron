 ...
 **/
static Point
computeEndTangent(const Point& first,
                  const Point& second)
{
    Point ret;
 ...
 **/
static Point
computeCenterTangent(const Point& prev,
                     const Point& center,
                     const Point& next)
{
    Point v1, v2, ret;
 ...
 **/
static double
newtonRaphsonRootFind(const std::vector<Point>& Q,
                      const Point& P,
                      double u)
{
 ...
 **/
static void
reparameterize(const std::vector<Point> &points,
               const std::vector<SimpleBezierCP>& bezCurve,
               std::vector<double>& u)
{
...
computeMaxError(const std::vector<Point>& points,
                const std::vector<SimpleBezierCP>& bezierCurve,
 ...
 **/
static void
chordLengthParametrize(const std::vector<Point>& points,
                       std::vector<double>* u)
{
...
static void
generateBezier(const std::vector<Point>& points,
               const std::vector<double>& u,
               const Point& tHat1,
               const Point& tHat2,
...
static void
fit_cubic_internal(const std::vector<Point>& points,
                   const Point&  tHat1,
                   const Point& tHat2,
                   double error,
                   std::vector<SimpleBezierCP>* generatedBezier)
...
static void
fit_cubic_for_sub_set(const std::vector<Point>& points,
                      double error,
                      std::vector<SimpleBezierCP>* generatedBezier)
{
    if (points.size() == 1) {
...
void
FitCurve::fit_cubic(const std::vector<Point>& points,
                    double error,
                    std::vector<SimpleBezierCP>* generatedBezier)
{
    if (points.size() < 2) {
