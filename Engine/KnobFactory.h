...
public:
    ...
    template <typename K>
    boost::shared_ptr<K> createKnob(boost::shared_ptr<KnobHolder>  holder, const std::string &description, int dimension = 1) const
    {
        return boost::dynamic_pointer_cast<K>(createKnob(K::typeNameStatic(),holder,description,dimension));
    }

private:
    boost::shared_ptr<KnobHelper> createKnob(const std::string &id, const boost::shared_ptr<KnobHolder>& holder,
