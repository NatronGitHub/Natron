    ...
    switch (overloadId) {
        case 0: // random(double min, double max) const
        {
            ...
            if (!PyErr_Occurred()) {
                ...
                double cppResult = const_cast<const ::Param*>(cppSelf)->random(cppArg0, cppArg1);
        ...
        case 1: // random(unsigned int seed) const
        {
            ...
            if (!PyErr_Occurred()) {
                // random(uint)const
                double cppResult = const_cast<const ::Param*>(cppSelf)->random(cppArg0);
                pyResult = Shiboken::Conversions::copyToPython(Shiboken::Conversions::PrimitiveTypeConverter<double>(), &cppResult);
            }
            break;
        }
    ...
    switch (overloadId) {
        ...
        case 1: // randomInt(unsigned int seed) const
        {
            ...
            if (!PyErr_Occurred()) {
                ...
                int cppResult = const_cast<const ::Param*>(cppSelf)->randomInt(cppArg0);
                pyResult = Shiboken::Conversions::copyToPython(Shiboken::Conversions::PrimitiveTypeConverter<int>(), &cppResult);
            }
            break;
        }
