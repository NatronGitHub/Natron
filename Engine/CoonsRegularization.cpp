           ...
           double t)
{
    ...
    if (t == t_i) {
        ...
        std::advance(it, t_i);
        Point ret;
        (*it)->getPositionAtTime(time, ViewIdx(0), &ret.x, &ret.y);
    ...
    Point p0, p1, p2, p3;
    (*it)->getPositionAtTime(time, ViewIdx(0), &p0.x, &p0.y);
    (*it)->getRightBezierPointAtTime(time, ViewIdx(0), &p1.x, &p1.y);
               ...
               double t)
{
    ...
    if (t == t_i) {
        ...
        std::advance(it, t_i);
        Point ret;
        (*it)->getLeftBezierPointAtTime(time, ViewIdx(0), &ret.x, &ret.y);
    ...
    Point ret;
    (*it)->getPositionAtTime(time, ViewIdx(0), &a.x, &a.y);
    (*it)->getRightBezierPointAtTime(time, ViewIdx(0), &b.x, &b.y);
    (*next)->getLeftBezierPointAtTime(time, ViewIdx(0), &c.x, &c.y);
    ab.x = (1. - t) * a.x + t * b.x;
    ab.y = (1. - t) * a.y + t * b.y;
                ...
                double t)
{
    ...
    if (t == t_i) {
        ...
        std::advance(it, t_i);
        Point ret;
        (*it)->getRightBezierPointAtTime(time, ViewIdx(0), &ret.x, &ret.y);
    ...
    Point a, b, c, ab, bc, abc;
    Point ret;
    (*it)->getRightBezierPointAtTime(time, ViewIdx(0), &a.x, &a.y);
    (*next)->getLeftBezierPointAtTime(time, ViewIdx(0), &b.x, &b.y);
    (*next)->getPositionAtTime(time, ViewIdx(0), &c.x, &c.y);
    ab.x = (1. - t) * a.x + t * b.x;
    ab.y = (1. - t) * a.y + t * b.y;
    ...
    if ( (abc.x == c.x) && (abc.y == c.y) ) {
        (*next)->getRightBezierPointAtTime(time, ViewIdx(0), &ret.x, &ret.y);
                 ...
                 int* before)
{
    ...
    for (; s1 != cps.end(); ++s1, ++s2, ++index) {
        ...
        Point z0, z1, c0, c1;
        (*s1)->getPositionAtTime(time, ViewIdx(0), &z0.x, &z0.y);
        (*s1)->getRightBezierPointAtTime(time, ViewIdx(0), &c0.x, &c0.y);
        (*s2)->getPositionAtTime(time, ViewIdx(0), &z1.x, &z1.y);
        (*s2)->getLeftBezierPointAtTime(time, ViewIdx(0), &c1.x, &c1.y);
        ...
        for (std::size_t i = 0; i < roots.size(); ++i) {
            if ( (roots[i] >= -fuzz) && (roots[i] <= 1. + fuzz) ) {
                ...
                for (std::vector<std::pair<BezierCPPtr, std::pair<BezierCPs::const_iterator, BezierCPs::const_iterator> > >::iterator it = intersections.begin(); it != intersections.end(); ++it) {
                    Point other;
                    it->first->getPositionAtTime(time, ViewIdx(0), &other.x, &other.y);
           ...
           Point ret[4][4])
{
    ...
    for (int j = 0; j < 4; ++j,
        ...
        Point p1;
        (*cur)->getPositionAtTime(time, ViewIdx(0), &p1.x, &p1.y);

        Point p1left;
        (*cur)->getLeftBezierPointAtTime(time, ViewIdx(0), &p1left.x, &p1left.y);

        Point p1right;
        (*cur)->getRightBezierPointAtTime(time, ViewIdx(0), &p1right.x, &p1right.y);

        Point p0;
        (*prev)->getPositionAtTime(time, ViewIdx(0), &p0.x, &p0.y);

        Point p2;
        (*next)->getPositionAtTime(time, ViewIdx(0), &p2.x, &p2.y);

        Point p0left;
        (*prev)->getLeftBezierPointAtTime(time, ViewIdx(0), &p0left.x, &p0left.y);

        Point p2right;
        (*next)->getRightBezierPointAtTime(time, ViewIdx(0), &p2right.x, &p2right.y);

        Point p3;
        (*nextNext)->getPositionAtTime(time, ViewIdx(0), &p3.x, &p3.y);
                ...
                double time)
{
    ...
    for (BezierCPs::const_iterator it = cps.begin(); it != cps.end(); ++it, ++i) {
        ...
        Point p;
        (*it)->getPositionAtTime(time, ViewIdx(0), &p.x, &p.y);
        Point q;
        q.x = p.x;
        q.y = p.y + dir.y;
        BezierCPPtr newPoint;
        ...
        if ( newPoint && (beforeIndex != -1) ) {
            Point np;
            newPoint->getPositionAtTime(time, ViewIdx(0), &np.x, &np.y);
    ...
    Point ret;
    cps.front()->getPositionAtTime(time, ViewIdx(0), &ret.x, &ret.y);
                                ...
                                std::list<BezierCPs> *fixedPatch)
{
    ...
    int sign;
    {
        ...
        bbox.y1 = std::numeric_limits<double>::infinity();
        bbox.y2 = -std::numeric_limits<double>::infinity();
        Bezier::bezierSegmentListBboxUpdate(patch, true, false, time, ViewIdx(0), 0, Transform::Matrix3x3(), &bbox);
