...
static Point getPointAt(const BezierCPs& cps, double time, double t)
{
    ...
    if (t == t_i) {
        ...
        std::advance(it, t_i);
        Point ret;
        (*it)->getPositionAtTime(false ,time,/*view*/0, &ret.x, &ret.y);
        return ret;
...
static Point getLeftPointAt(const BezierCPs& cps, double time, double t)
{
    ...
    if (t == t_i) {
        ...
        std::advance(it, t_i);
        Point ret;
        (*it)->getLeftBezierPointAtTime(false ,time, /*view*/0,&ret.x, &ret.y);
        return ret;
    } else if (t == t_i_plus_1) {
        BezierCPs::const_iterator it = cps.begin();
        std::advance(it, t_i_plus_1);
    ...
    Point ret;
    (*it)->getPositionAtTime(false ,time, /*view*/0,&a.x, &a.y);
    (*it)->getRightBezierPointAtTime(false ,time, /*view*/0,&b.x, &b.y);
    (*next)->getLeftBezierPointAtTime(false ,time,/*view*/0, &c.x, &c.y);
    ab.x = (1. - t) * a.x + t * b.x;
    ab.y = (1. - t) * a.y + t * b.y;
...
static Point getRightPointAt(const BezierCPs& cps, double time, double t)
{
    ...
    if (t == t_i) {
        ...
        std::advance(it, t_i);
        Point ret;
        (*it)->getRightBezierPointAtTime(false ,time, /*view*/0,&ret.x, &ret.y);
        return ret;
    } else if (t == t_i_plus_1) {
        BezierCPs::const_iterator it = cps.begin();
        std::advance(it, t_i_plus_1);
    ...
    Point a,b,c,ab,bc,abc;
    Point ret;
    (*it)->getRightBezierPointAtTime(false ,time,/*view*/0, &a.x, &a.y);
    (*next)->getLeftBezierPointAtTime(false ,time, /*view*/0,&b.x, &b.y);
    (*next)->getPositionAtTime(false ,time, /*view*/0,&c.x, &c.y);
    ab.x = (1. - t) * a.x + t * b.x;
    ab.y = (1. - t) * a.y + t * b.y;
    ...
    if (abc.x == c.x && abc.y == c.y) {
        (*next)->getRightBezierPointAtTime(false ,time,/*view*/0, &ret.x, &ret.y);
        return ret;
    } else {
        return abc;
    }
                             ...
                             int* before)
{
    ...
    for (; s1 != cps.end(); ++s1,++s2, ++index) {
        ...
        Point z0,z1,c0,c1;
        (*s1)->getPositionAtTime(false ,time, /*view*/0,&z0.x, &z0.y);
        (*s1)->getRightBezierPointAtTime(false ,time, /*view*/0,&c0.x, &c0.y);
        (*s2)->getPositionAtTime(false ,time, /*view*/0,&z1.x, &z1.y);
        (*s2)->getLeftBezierPointAtTime(false ,time,/*view*/0, &c1.x, &c1.y);
        ...
        for (std::size_t i = 0; i < roots.size(); ++i) {
            if (roots[i] >= -fuzz && roots[i] <= 1. + fuzz) {
                ...
                for (std::vector<std::pair<boost::shared_ptr<BezierCP>,std::pair<BezierCPs::const_iterator,BezierCPs::const_iterator> > >::iterator it = intersections.begin(); it!=intersections.end(); ++it) {
                    Point other;
                    it->first->getPositionAtTime(false ,time, /*view*/0,&other.x, &other.y);
...
static void coonsPatch(const BezierCPs& p, double time, Point ret[4][4])
{
    ...
    for (int j = 0; j < 4; ++j,
        ...
        Point p1;
        (*cur)->getPositionAtTime(false ,time, /*view*/0,&p1.x, &p1.y);
        
        Point p1left;
        (*cur)->getLeftBezierPointAtTime(false ,time, /*view*/0,&p1left.x, &p1left.y);
        
        Point p1right;
        (*cur)->getRightBezierPointAtTime(false ,time, /*view*/0,&p1right.x, &p1right.y);
        
        Point p0;
        (*prev)->getPositionAtTime(false ,time, /*view*/0,&p0.x, &p0.y);
        
        Point p2;
        (*next)->getPositionAtTime(false ,time, /*view*/0,&p2.x, &p2.y);
        
        Point p0left;
        (*prev)->getLeftBezierPointAtTime(false ,time, /*view*/0,&p0left.x, &p0left.y);
        
        Point p2right;
        (*next)->getRightBezierPointAtTime(false ,time,/*view*/0, &p2right.x, &p2right.y);
        
        Point p3;
        (*nextNext)->getPositionAtTime(false ,time, /*view*/0,&p3.x, &p3.y);
...
Point findPointInside(const BezierCPs& cps, double time)
{
    ...
    for (BezierCPs::const_iterator it = cps.begin(); it != cps.end(); ++it, ++i) {
        ...
        if (newPoint && beforeIndex != -1) {
            Point np;
            newPoint->getPositionAtTime(false ,time,/*view*/0, &np.x, &np.y);
    ...
    Point ret;
    cps.front()->getPositionAtTime(false ,time, /*view*/0,&ret.x, &ret.y);
    return ret;
}
...
CoonsRegularization::regularize(const BezierCPs &patch, double time, std::list<BezierCPs> *fixedPatch)
{
    ...
    int sign;
    {
        ...
        bbox.y1 = std::numeric_limits<double>::infinity();
        bbox.y2 = -std::numeric_limits<double>::infinity();
        Bezier::bezierSegmentListBboxUpdate(false ,patch, true, false, time, /*view*/0 ,0, Transform::Matrix3x3(), &bbox);
