...
CustomParamInteract::paintGL()
{
     ...
     */
    {
        ...
        _imp->entryPoint->drawAction(time, scale, /*view=*/ 0, /*colourPicker=*/NULL);
        glCheckError();
    } // GLProtectAttrib a(GL_TRANSFORM_BIT);
}
...
CustomParamInteract::mousePressEvent(QMouseEvent* e)
{
    ...
    viewportPos.y = e->x();
    viewportPos.y = e->y();
    OfxStatus stat = _imp->entryPoint->penDownAction(time, scale, /*view=*/ 0, /*colourPicker=*/NULL, pos, viewportPos, /*pressure=*/ 1.);
    if (stat == kOfxStatOK) {
        update();
    }
}
...
CustomParamInteract::mouseMoveEvent(QMouseEvent* e)
{
    ...
    viewportPos.y = e->x();
    viewportPos.y = e->y();
    OfxStatus stat = _imp->entryPoint->penMotionAction(time, scale, /*view=*/ 0, /*colourPicker=*/NULL, pos, viewportPos, /*pressure=*/ 1.);
    if (stat == kOfxStatOK) {
        update();
    }
}
...
CustomParamInteract::mouseReleaseEvent(QMouseEvent* e)
{
    ...
    viewportPos.y = e->x();
    viewportPos.y = e->y();
    OfxStatus stat = _imp->entryPoint->penUpAction(time, scale, /*view=*/ 0, /*colourPicker=*/NULL, pos, viewportPos, /*pressure=*/ 1.);
    if (stat == kOfxStatOK) {
        update();
    }
}
...
CustomParamInteract::focusInEvent(QFocusEvent* /*e*/)
{
    ...
    scale.x = scale.y = 1.;
    double time = _imp->knob.lock()->getKnob()->getHolder()->getApp()->getTimeLine()->currentFrame();
    OfxStatus stat = _imp->entryPoint->gainFocusAction(time, scale, /*view=*/ 0, /*colourPicker=*/NULL);
    if (stat == kOfxStatOK) {
        update();
    }
}
...
CustomParamInteract::focusOutEvent(QFocusEvent* /*e*/)
{
    ...
    scale.x = scale.y = 1.;
    double time = _imp->knob.lock()->getKnob()->getHolder()->getApp()->getTimeLine()->currentFrame();
    OfxStatus stat = _imp->entryPoint->loseFocusAction(time, scale, /*view=*/ 0, /*colourPicker=*/NULL);
    if (stat == kOfxStatOK) {
        update();
    }
}
...
CustomParamInteract::keyPressEvent(QKeyEvent* e)
{
    ...
    OfxStatus stat;
    if ( e->isAutoRepeat() ) {
        stat = _imp->entryPoint->keyRepeatAction( time, scale, /*view=*/ 0, /*colourPicker=*/NULL, (int)QtEnumConvert::fromQtKey( (Qt::Key)e->key() ), keyStr.data() );
    } else {
        stat = _imp->entryPoint->keyDownAction( time, scale, /*view=*/ 0, /*colourPicker=*/NULL, (int)QtEnumConvert::fromQtKey( (Qt::Key)e->key() ), keyStr.data() );
    }
    if (stat == kOfxStatOK) {
        update();
    }
...
CustomParamInteract::keyReleaseEvent(QKeyEvent* e)
{
    ...
    double time = _imp->knob.lock()->getKnob()->getHolder()->getApp()->getTimeLine()->currentFrame();
    QByteArray keyStr;
    OfxStatus stat = _imp->entryPoint->keyUpAction( time, scale, /*view=*/ 0, /*colourPicker=*/NULL, (int)QtEnumConvert::fromQtKey( (Qt::Key)e->key() ), keyStr.data() );
    if (stat == kOfxStatOK) {
        update();
    }
}
