...
SetKnobChangedCallbackCommand::SetKnobChangedCallbackCommand(const boost::shared_ptr<NodeGui> & node,
...
SetKnobChangedCallbackCommand::~SetKnobChangedCallbackCommand()
{
    
}

void
SetKnobChangedCallbackCommand::setScript(const std::string& script)
{
    boost::shared_ptr<NodeGui> node = _node.lock();
    if (!node) {
        return;
    }
    try {
        node->getNode()->getLiveInstance()->setKnobChangedCallback(script);
    } catch (...) {
        Natron::errorDialog(QObject::tr("Script").toStdString(), QObject::tr("The Script is invalid").toStdString());
    }
    node->getDagGui()->getGui()->getApp()->triggerAutoSave();
    setText(QObject::tr("Set paramChanged callback"));
}

void SetKnobChangedCallbackCommand::undo()
{
    setScript(_oldScript);
}

void SetKnobChangedCallbackCommand::redo()
{
    setScript(_newScript);
}